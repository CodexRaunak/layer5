...
When you run `np skill install`, the package lands in `~/.agents/skills/`. Claude Code scans `~/.claude/skills/` at startup and finds nothing new. The two directories never communicate with each other by default.
